...
class LpLoss(object):
    ...
    def __init__(self, d=1, p=2, measure=1., reduction='mean'):
        super().__init__()
    ...
    def rel(self, x, y):
        ...
        diff = torch.norm(torch.flatten(x, start_dim=-1) - torch.flatten(y, start_dim=-1),
                          p=self.p, dim=-1, keepdim=False)
        ynorm = torch.norm(torch.flatten(y, start_dim=-1),
                           p=self.p, dim=-1, keepdim=False)

        epsilon = 1e-8
        diff = diff/(ynorm + epsilon)
...
class H1Loss(object):
    ...
    def __init__(self, d=1, measure=1., reduction='mean', fix_x_bnd=False, fix_y_bnd=False, fix_z_bnd=False):
        super().__init__()
    ...
    def compute_terms(self, x, y, quadrature):
        ...
        if self.d == 1:
            ...
            dict_x[0] = torch.flatten(x, start_dim=-1)
            dict_y[0] = torch.flatten(y, start_dim=-1)

            x_x, x_y = central_diff_2d(
                x, quadrature, fix_x_bnd=self.fix_x_bnd, fix_y_bnd=self.fix_y_bnd)
            y_x, y_y = central_diff_2d(
                y, quadrature, fix_x_bnd=self.fix_x_bnd, fix_y_bnd=self.fix_y_bnd)

            dict_x[1] = torch.flatten(x_x, start_dim=-1)
            dict_x[2] = torch.flatten(x_y, start_dim=-1)

            dict_y[1] = torch.flatten(y_x, start_dim=-1)
            dict_y[2] = torch.flatten(y_y, start_dim=-1)
            ...
            x_x, x_y, x_z = central_diff_3d(
                x, quadrature, fix_x_bnd=self.fix_x_bnd, fix_y_bnd=self.fix_y_bnd, fix_z_bnd=self.fix_z_bnd)
            y_x, y_y, y_z = central_diff_3d(
                y, quadrature, fix_x_bnd=self.fix_x_bnd, fix_y_bnd=self.fix_y_bnd, fix_z_bnd=self.fix_z_bnd)
    ...
    def abs(self, x, y, quadrature=None):
        ...
        diff = const*torch.norm(dict_x[0] - dict_y[0],
                                p=2, dim=-1, keepdim=False)**2

        for j in range(1, self.d + 1):
            diff += const * \
                torch.norm(dict_x[j] - dict_y[j], p=2,
                           dim=-1, keepdim=False)**2
    ...
    def rel(self, x, y, quadrature=None):
        ...
        for j in range(1, self.d + 1):
            diff += torch.norm(dict_x[j] - dict_y[j],
                               p=2, dim=-1, keepdim=False)**2
            ynorm += torch.norm(dict_y[j], p=2, dim=-1, keepdim=False)**2

        epsilon = 1e-8
        diff = (diff**0.5)/(ynorm**0.5 + epsilon)
...
class H1WMSELoss(nn.Module):
    ...
    def __init__(self, d, reduction='mean'):
        ...
        """
        super(H1WMSELoss, self).__init__()

        self.factor = 10
        self.reduction = reduction

        self.h1loss = H1Loss(d=d)
        self.wmseloss = WMSELoss()

    def forward(self, inputs, targets):
        loss = self.h1loss(inputs, targets) + self.wmseloss(inputs, targets)
...
class WMSELoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        if flood_inputs.numel() == 0 or flood_targets.numel() == 0:
            flood_loss = torch.tensor(0.0).cuda()
        else:
...
class R2Metric(object):
    ...
    def __call__(self, y_pred, y, **kwargs):
        ...
        if self.reduction == 'sum':
            ...
            self.ss_res += torch.mean(ss_res).item()
            self.ss_tot += torch.mean(ss_tot).item()
...
class PointwiseQuantileLoss(object):
    ...
    def __call__(self, y_pred, y, eps=1e-7, **kwargs):
        ...
        ptwise_loss_scaled = ptwise_loss / 2 / \
            quantile / (1 - quantile) / yscale
        ptavg_loss = ptwise_loss_scaled.view(
            ptwise_loss_scaled.shape[0], -1).mean(1, keepdim=True)
