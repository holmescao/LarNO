...
def CGELU(x: torch.Tensor):
    ...
    return F.gelu(x.real).type(torch.float) + 1j * F.gelu(x.imag).type(
        torch.float
    )
...
def ctanh(x: torch.Tensor):
    ...
    Args:
        x: complex tensor
    """
    return torch.tanh(x.real).type(torch.float) + 1j * torch.tanh(x.imag).type(
        torch.float
    )


def apply_complex(real_func, imag_func, x, dtype=torch.float):
    """
